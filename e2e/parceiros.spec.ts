...
test.describe("Gestão de parceiros (admin)", () => {
  ...
  test("abre o modal de detalhes ao clicar no card do parceiro", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Ver detalhes de NutriVida" }).click();
  ...
  test("clicar em 'Editar' não abre o modal de detalhes", async ({ page }) => {
    await page.getByRole("button", { name: "Editar" }).first().click();
    await expect(page.getByRole("dialog").getByText("Editar parceiro")).toBeVisible();
  });

  test("cadastra um novo parceiro com categoria digitada livremente", async ({ page }) => {
    ...
    await dialog.getByLabel("Categoria").fill("Odontologia");
    await dialog.getByRole("button", { name: "Cadastrar" }).click();

    await expect(dialog).not.toBeVisible();
    await expect(page.getByText("Parceiro E2E")).toBeVisible();
    await expect(page.getByText("Odontologia", { exact: true })).toBeVisible();
  });
