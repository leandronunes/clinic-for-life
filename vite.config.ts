...
const config = {
  ...
  test: {
    ...
    // real Pact mock server) — see npm run test:pact. e2e/**/*.spec.ts and
    // smoke/**/*.spec.ts run under Playwright (npm run test:e2e / test:smoke),
    // a real browser, not jsdom/vitest — matches Vitest's default *.spec.ts
    // include glob, so must be excluded.
    exclude: [...configDefaults.exclude, ".claude/**", "**/*.pact.test.ts", "e2e/**", "smoke/**"],
  },
};
